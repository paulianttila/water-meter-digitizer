...
class ReadConfig:
    def __init__(
        self,
        _path,
        _pathdefault="./config_default/",
        _configreroute="./config/",
        copydummyconfig=True,
    ):
    ...
    def ParseConfig(self):
        ...
        if self.Analog_DoLog:
            ...
            self.Analog_LogNames = ""
            self.Analog_LogName_Enabled = config.has_option(
                "Analog_Counter", "LogNames"
            )
            if self.Analog_LogName_Enabled:
                self.Analog_LogNamesFull = config.get(
                    "Analog_Counter", "LogNames"
                ).split(",")
                self.Analog_LogNames = []
        ...
        if config.has_option("ConsistencyCheck", "AllowNegativeRates"):
            self.Zaehler_AllowNegativeRates = config["ConsistencyCheck"][
                "AllowNegativeRates"
            ]
        ...
        if config.has_option("ConsistencyCheck", "MaxRateValue"):
            self.Zaehler_MaxRateValue = float(
                config["ConsistencyCheck"]["MaxRateValue"]
            )
        ...
        if config.has_option("ConsistencyCheck", "ReadPreValueFromFileMaxAge"):
            self.Zaehler_ReadPreValueFromFileMaxAge = int(
        ...
        if config.has_option("ConsistencyCheck", "ReadPreValueFromFileAtStartup"):
            self.Zaehler_ReadPreValueFromFileAtStartup = config["ConsistencyCheck"][
                "ReadPreValueFromFileAtStartup"
            ]
        ...
        self.HTTP_log_Image = ""
        self.LogImageLocation_Enabled = config.has_option(
            "Imagesource", "LogImageLocation"
        )
        ...
        self.HTTP_LogOnlyFalsePictures_Enabled = config.has_option(
            "Imagesource", "LogOnlyFalsePictures"
        )
        if self.HTTP_LogOnlyFalsePictures_Enabled:
            self.HTTP_LogOnlyFalsePictures = bool(
        ...
        self.Cut_reference_pos.append(
            (
                int(config["alignment.ref0"]["pos_x"]),
                int(config["alignment.ref0"]["pos_y"]),
            )
        )
        self.Cut_reference_pos.append(
            (
                int(config["alignment.ref1"]["pos_x"]),
                int(config["alignment.ref1"]["pos_y"]),
            )
        )
        self.Cut_reference_pos.append(
            (
                int(config["alignment.ref2"]["pos_x"]),
                int(config["alignment.ref2"]["pos_y"]),
            )
        )
    ...
    def LoadHTTPParameter(self):
        return (
            self.HTTP_TimeoutLoadImage,
            self.HTTP_URLImageSource,
            self.HTTP_log_Image,
            self.HTTP_LogOnlyFalsePictures,
        )
    ...
    def ZaehlerConsistency(self):
        return (
            self.Zaehler_ConsistencyEnabled,
            self.Zaehler_AllowNegativeRates,
            self.Zaehler_MaxRateValue,
            self.Zaehler_ErrorReturn,
        )

    def ZaehlerReadPrevalue(self):
        return (
            self.Zaehler_ReadPreValueFromFileAtStartup,
            self.Zaehler_ReadPreValueFromFileMaxAge,
        )
