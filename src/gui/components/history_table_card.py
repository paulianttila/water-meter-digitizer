...
class HistoryTableCard:
    ...
    def render(self, container: ui.column) -> None:
        ...
        def refresh_table() -> None:
            ...
            raw_readings = storage.get_readings(
            ...
            with container:
                ...
                with ui.row().classes(
                    ...
                    with ui.row().classes("items-center gap-2 flex-wrap"):
                        ...
                        def on_export_csv() -> None:
                            ...
                            ui.notify(
                                f"Exported {len(raw_readings)} readings to {filename}",
                                type="positive",
                            )
                        ...
                        def on_export_json() -> None:
                            if not raw_readings:
                                ...
                                return
                            export_data = [
                                r.model_dump(mode="json") for r in raw_readings
                            ]
                        ...
                        ui.button(icon="refresh", on_click=refresh_table).props(
                            "flat dense round color=cyan"
                ...
                # 2. KPI Summary Bar (computed on the loaded raw dataset)
                total_loaded = len(raw_readings)
                good_count = sum(
                    1
                    for r in raw_readings
                    if not r.error
                ...
                flow_count = sum(1 for r in raw_readings if r.flow_detected)
                snapshot_count = sum(
                    1 for r in raw_readings if r.frame_path or r.frame_type
                )
                ...
                with ui.row().classes("w-full gap-4 flex-wrap my-1"):
                    ...
                    with ui.element("div").classes(
                        ...
                        with ui.row().classes("items-baseline gap-1.5 mt-0.5"):
                            ui.label(f"{len(filtered_readings)}").classes(
                                "font-['Outfit'] text-2xl font-bold text-cyan-300"
                            )
                            if len(filtered_readings) != total_loaded:
                                ui.label(f"/ {total_loaded}").classes(
                    ...
                    with ui.element("div").classes(
                        ...
                        with ui.row().classes("items-baseline gap-1.5 mt-0.5"):
                            ...
                            ui.label(f"{health_pct:.1f}%").classes(
                                f"font-['Outfit'] text-2xl font-bold {health_color}"
                            )
                            ui.label("Good").classes(
                    ...
                    with ui.element("div").classes(
                        ...
                        with ui.row().classes("items-baseline gap-1.5 mt-0.5"):
                            ui.label(f"{avg_confidence:.1f}%").classes(
                    ...
                    with ui.element("div").classes(
                        ...
                        with ui.row().classes("items-baseline gap-3 mt-0.5"):
                            with ui.row().classes("items-center gap-1"):
                                ui.icon("water_drop", size="xs").classes(
                                    "text-blue-400"
                                )
                                ui.label(f"{flow_count}").classes(
                                    "font-['Outfit'] text-xl font-bold text-blue-300"
                                )
                            with ui.row().classes("items-center gap-1"):
                                ui.icon("photo_camera", size="xs").classes(
                                    "text-teal-400"
                                )
                                ui.label(f"{snapshot_count}").classes(
                ...
                # 3. Quasar Table with Custom Visual Slots
                with ui.element("div").classes(
                    ...
                    def on_inspect_reading(e: Any) -> None:
                        ...
                        if reading_id in reading_map:
                            self.open_reading_dialog(reading_map[reading_id])

                    table.on("inspect_reading", on_inspect_reading)
